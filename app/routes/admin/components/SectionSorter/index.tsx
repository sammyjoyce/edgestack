...
export default function SectionSorter({
	...
	const [orderedItems, setOrderedItems] = React.useState<SectionDetail[]>(
		() => {
			...
			return (sectionDetailsOrdered || []).sort(
				(a, b) => dbOrder.indexOf(a.id) - dbOrder.indexOf(b.id),
			);
		},
	);
	...
	React.useEffect(() => {
		const dbOrder = (initialSectionsFromDb || []).map((s) => s.id);
		const currentSortedDetails = [...(sectionDetailsOrdered || [])].sort(
			(a, b) => dbOrder.indexOf(a.id) - dbOrder.indexOf(b.id),
		);
	...
	React.useEffect(() => {
		const initialOrderString = (initialSectionsFromDb || [])
			.map((s) => s.id)
			.join(",");
		const currentOrder = orderedItems.map((s) => s.id).join(",");
		if (initialOrderString !== currentOrder && orderFetcher.state === "idle") {
		...
		>
			...
			<div className="flex items-center">
				...
				{themeErrorForThisItem && (
					<Alert
						variant="error"
						className="mt-1 w-full text-xs text-right"
						showIcon={false}
					>
