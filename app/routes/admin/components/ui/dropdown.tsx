...
export function DropdownButton<T extends React.ElementType = typeof Button>({
  as = Button as T, // Ensure 'as' prop is correctly typed
  ...props
...
export function DropdownItem({
  ...
  return 'href' in props ? (
    ...
    <Headless.MenuItem as="button" type="button" {...props} className={itemClasses} />
  )
}

export function DropdownHeader({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return <div {...props} className={clsx(className, 'px-2.5 py-1 text-xs font-semibold text-zinc-500 dark:text-zinc-400')} />
}
...
export function DropdownLabel({
  ...
  return (
    <Headless.Label {...props} data-slot="label" className={clsx(className, 'flex-1 truncate')} />
  )
}
...
export function DropdownShortcut({
  ...
  return (
    <Headless.Description
      as="kbd"
      {...props}
      className={clsx(className, 'ml-auto text-xs text-zinc-400 group-data-[focus]:text-primary-100 dark:group-data-[focus]:text-primary-dark-100')}
    >
