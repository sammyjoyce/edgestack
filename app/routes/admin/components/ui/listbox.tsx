...
export function Listbox<TValue>({ // Changed T to TValue for clarity
  className,
  placeholder,
  autoFocus,
...
} & Omit<Headless.ListboxProps<typeof Fragment, TValue>, 'as' | 'multiple' | 'value' | 'onChange'>) {
  return (
    <Headless.Listbox {...props} value={value} onChange={onChange} multiple={false}>
      ...
      >
        <Headless.ListboxSelectedOption
          as="span"
          options={options}
          placeholder={placeholder && <span className="block truncate text-zinc-400 dark:text-zinc-500">{placeholder}</span>}
...
export function ListboxOption<TValue>({ // Changed T to TValue
  children,
  className,
  value, // Added value prop
  ...props
  ...
  children?: React.ReactNode;
  value: TValue; // Explicitly define value prop
} & Omit<Headless.ListboxOptionProps<'div', TValue>, 'as' | 'className' | 'value'>) {
  return (
      ...
      {({ selected, active }) => (
        <div // This div is the actual rendered option item
          className={clsx(
            ...
            active ? 'bg-primary/10 text-primary dark:bg-primary-dark/20 dark:text-primary-dark' : 'text-zinc-700 dark:text-zinc-200',
            selected ? 'font-semibold' : 'font-normal',
            'data-disabled:opacity-50 data-disabled:cursor-not-allowed'
          )}
        >
          {selected && <CheckIcon />} 
          <span className={clsx('flex-1 truncate', selected ? 'pl-1' : 'pl-5')}>{children}</span>
        </div>
      )}
    </Headless.ListboxOption>
  );
