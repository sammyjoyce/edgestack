...
export function Dialog({
  ...
  ...props
}: { size?: keyof typeof sizes; className?: string; children: React.ReactNode; onClose: () => void } & Omit<
...
export function DialogTitle({
  ...
  return (
    <Headless.DialogTitle
      {...props}
      className={clsx(className, 'text-xl font-semibold text-zinc-900 dark:text-white')}
    />
  )
}
...
export function DialogDescription({
  ...
  return <Headless.Description as={Text} {...props} className={clsx(className, 'mt-2 text-zinc-600 dark:text-zinc-400')} />
}
